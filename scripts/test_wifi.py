#!/usr/bin/env python3
"""Run automated WiFi diagnostic test on connected Lilygo ESP32 using mpremote."""
...
def run_wifi_test(
    ...
    """Upload local config and execute WiFi diagnostic test on ESP32."""
    repo_root = Path(__file__).resolve().parent.parent
    firmware_dir = repo_root / "firmware"
    test_script = firmware_dir / "test_wifi.py"

    if not test_script.exists():
        print(f"[test_wifi] ERROR: Test script not found at {test_script}")
        return 1
    ...
    if sync_config:
        ...
        if found_config:
            print(f"[test_wifi] Syncing {found_config.name} to ESP32...")
            cp_cmd = [*base_cmd, "cp", str(found_config), f":{found_config.name}"]
            if dry_run:
                print(f"[test_wifi] (Dry run) {' '.join(cp_cmd)}")
            else:
                try:
                    subprocess.run(cp_cmd, check=True)
                except subprocess.CalledProcessError as exc:
                    print(f"[test_wifi] Warning: Failed to sync config: {exc}")

    # Run test_wifi.py on device
    run_cmd = [*base_cmd, "run", str(test_script)]
    print(f"[test_wifi] Running WiFi test on ESP32: {' '.join(run_cmd)}")

    if dry_run:
        print("[test_wifi] (Dry run - test not executed)")
        return 0
    ...
    try:
        ...
        print("[test_wifi] ERROR: mpremote is not installed in the environment.")
        return 1
    except KeyboardInterrupt:
        print("\n[test_wifi] Aborted by user.")
        return 0


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Lilygo ESP32 WiFi Diagnostic Runner",
    ...
    if not port and not args.dry_run:
        ...
        if port:
            print(f"[test_wifi] Auto-detected serial port: {port}")
        else:
            print(
                "[test_wifi] Warning: No serial port auto-detected (/dev/ttyACM* or /dev/ttyUSB*)."
            )
